...
class PlainMarkerProfile(ProfileBase):
    ...
    def _finalize_entry(self, entry_dict: dict, metadata: dict) -> BundleEntry:
        ...
        if is_binary:
            # For binary files, the content is already a base64 string. Just strip incidental whitespace.
            content = content.strip()
        else:
            # Preserve text content verbatim so trailing newlines are maintained, but
            # treat a lone separator newline as empty content.
            if content in {'\n', '\r\n', '\r'}:
                content = ''
    ...
    def format_manifest(self, manifest: BundleManifest) -> str:
        ...
        for entry in manifest.entries:
            ...
            # Content handling
            if entry.is_binary:
                # Binary: content is already base64, just ensure it's clean
                output_lines.append(entry.content.strip())
            else:
                # Text: preserve content exactly, but strip single trailing newline
                # that would have been added by file I/O
                content = entry.content
                if content.endswith('\n'):
                    content = content[:-1]
                output_lines.append(content)
            
            # CRITICAL FIX: Always add blank line between entries
            output_lines.append('')  # This ensures proper separation
        
        # Remove the final trailing blank line after last entry
