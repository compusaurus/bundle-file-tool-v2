...
class PlainMarkerProfile(ProfileBase):
    ...
    def _finalize_entry(self, entry_dict: dict, metadata: dict) -> BundleEntry:
        ...
        # Strip trailing newlines more carefully:
        # - For non-empty content ending in newline(s), strip ONE trailing newline
        # - For empty content, leave as-is (empty string)
        # This preserves intentional trailing newlines while removing the parse artifact
        if content:
            # Only strip if content has at least one character before the newline
    ...
    def format_manifest(self, manifest: BundleManifest) -> str:
        ...
        for entry in manifest.entries:
            ...
            # Content - preserve exactly as is
            output_lines.append(entry.content)
            
            # Only add trailing newline if content doesn't already have one
